...
def get_answer_reports(telegram_id: int, year=0) -> List[dict]:
    ...
    data_one = {'year': year}
    if year_now == date.year: data_one.update({'start_month': date.month})
    result = [{
    ...
    if year == year_now:
        ...
        operations= get_answer_with_operations(telegram_id, date_start, date_end)
