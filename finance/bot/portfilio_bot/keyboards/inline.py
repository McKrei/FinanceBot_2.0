...
from bot.portfolio_bot.models.category import get_cat_by_tg
from bot.portfolio_bot.models.category import get_subcat_by_catid
from bot.portfolio_bot.models.category import check_for_one_sub
...
def inline_for_report_monthly(year: int = 0, start_month: int = 1) -> InlineKeyboardMarkup:
    ...
    but_year = InlineKeyboardButton(
        'За весь год', callback_data=f'r_ca_year_{year}-1-1')
...
def inline_for_operations(id_operation: int, type_operation: str) -> InlineKeyboardMarkup:
    ...
    buttons = [
        InlineKeyboardButton(
            'Отмена', callback_data=f'do_{type_operation}_{id_operation}'),
        InlineKeyboardButton(
            'Дата', callback_data=f'gdu_now_{type_operation}_{id_operation}')
    ]
...
def get_inline_button_report_period(date_start: dt.datetime,
    ...
    if type_report == 'category':
        ...
        inline.row(
            InlineKeyboardButton('Показать Подкатегории',
                                 callback_data=f'r_sc_{report_for}')
        )
    if report_for[0] == 'y':
        inline_list.append(
            InlineKeyboardButton(
                'График', callback_data=f'v_gr_{type_rep_short}_{report_for}')
        )
    else:
        inline_list += [
            InlineKeyboardButton(
                'Все операции', callback_data=f'allO_d_{report_for}'),
            InlineKeyboardButton('По дням', callback_data=f'byD_{report_for}')
        ]
    inline_list.append(
        InlineKeyboardButton(
            'Пирожок', callback_data=f'v_cr_{type_rep_short}_{report_for}')
    )
...
def get_inline_button_sum_operations_by_days(
    ...
    buttons_list = [
        InlineKeyboardButton(f'{day}. {bn(summa)} {cur}',
                             callback_data=f'allO_d_day_{year}-{month}-{day}')
