...
from bot.portfolio_bot.misc.util import get_words, get_number
from bot.portfolio_bot import keyboards as kb
from bot.portfolio_bot.misc.util import beautiful_numbers as bn
from bot.portfolio_bot.misc.util import create_detail_operation_sting
from bot.portfolio_bot.misc.util import beautiful_account_operation
from bot.portfolio_bot.models.currency import dict_currency_reduction_for_search
...
from bot.portfolio_bot.models.money import conversion_change_to_many
from bot.portfolio_bot.models.money import update_count_money
from bot.portfolio_bot.models.money import account_operation_change_to_many
...
def conversion_validator(msg: str) -> tuple:
    ...
    try:  # Проверка есть ли такая валюта
        cur = CurrencyReduction.objects.filter(
            reduction=cur_reduction)[0].currency
    except IndexError:
        return False, f'Нет такой валюты: {cur_reduction}\nпосмотреть список валют, команда: /currency_all'
    # Проверка есть ли число
    num = get_number(msg)
    if not num:
        return False, f'Не разобрал сумму сделки: {msg}'
...
def conversion_operation(telegram_id: int, message: str) -> Tuple[bool, str]:
    ...
    if not r_sell:
        return False, False, t_sell, None
    r_buy, t_buy = conversion_validator(buy)
    if not r_buy:
        return False, False, t_buy, None
    ...
    if not money:
        return False, False, f'У вас нету: {t_sell.name}.\nСперва нужно купить ее!', None
    price_basic_currency = money[0].conversion * t_sell[1]
    # Записываем изменения в БД
    old_course, result = conversion_change_to_many(
        user, price_basic_currency, *t_sell, *t_buy)
    id_operation = conversion_save(
        user, *t_sell, *t_buy, price_basic_currency, message)
...
def account_operation(telegram_id: int, message: str) -> Tuple[bool, str]:
    ...
    account_operation_change_to_many(
        money[0], summa_numbers, cat.income_or_expense)
...
def write_operation_callback(sub_id, summa, cur_id, word):
    ...
    answers = [
        {'text': mes, 'reply_markup': kb.inline_for_operations(id_operation, 'acc')}]
    if word:
        msg = f'Можно добавить ключ "{word}"\nДля быстрой записи в {sub.name.capitalize()}'
        answers += [{'text': msg,
                     'reply_markup': kb.inline_key_for_subcat(sub.id, word)}]
    return answers
